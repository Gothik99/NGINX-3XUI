# 🚀 NGINX + 3XUI: Автоустановщик Reverse Proxy
...
- 🔒 **SSL Сертификаты**: Автоматическая генерация через Let's Encrypt или использование уже имеющихся файлов
...
**Задача**: Скрыть конечный зарубежный сервер. Клиент подключается к RU-серверу, а тот перебрасывает трафик.

**Шаги**:

1. Запустите скрипт
2. Выберите в меню `1) Полная установка`
3. Введите домен этого сервера (например, `ru.myserver.com`)
4. Выберите роль `1) Только проксирующий (Relay)`
5. Введите адрес вашего зарубежного сервера (например, `de.myserver.com`)
6. Добавьте маршрут (например, gRPC с путем `/secret-grpc`)

**Готово!** В клиенте (v2rayNG) указывайте адрес `ru.myserver.com`, но ключи/UUID от зарубежного сервера.

---

### ⚙️ Сценарий 2: Backend-сервер (Зарубежный)

**Задача**: Поставить Nginx перед панелью 3XUI для использования порта 443 и маскировки под сайт.

**Шаги**:

1. Запустите скрипт на сервере, где установлена панель 3XUI
2. Выберите в меню `1) Полная установка`
3. Введите домен этого сервера
4. Выберите роль `2) Проксирующий под 3XUI (Backend)`
5. Добавьте маршрут:
   - **Транспорт**: gRPC
   - **Путь**: `/secret-grpc` (должен совпадать с настройками в панели)
   - **Локальный порт**: Например, `2053`

6. В панели 3XUI создайте Inbound:
   ```
   Port: 2053
   Listen IP: 127.0.0.1 (Важно! Только локально)
   Network: gRPC
   Service Name: /secret-grpc
   Security: None (Шифрованием занимается Nginx)
   ```
...
2. В главном меню выберите пункт `2) Добавить новый маршрут`
...
- Команде разработчиков [3XUI](https://github.com/MHSanaei/3x-ui)
